...
export const Route = createFileRoute("/recommendations")({
  head: () => ({
    meta: [
      {
        title:
          "AI-Powered Project Recommendations — CivilIntel",
      },
      {
        name: "description",
        content:
      ...
      },
      {
        ...
        content:
          "Know which development project to prioritise, and exactly why.",
      },
    ],
  }),
...
function scoreOf(r) {
  return Math.min(
    100,
    Math.round(r?.priority?.totalScore ?? 0),
  );
}
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <h3 className="mt-0.5 text-base font-bold">
              {rec.district} —{" "}
              {rec.recommendedProject?.replace(
                "Prioritize ",
                "",
              ) ?? rec.category}
            </h3>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 rounded-lg bg-success/10 px-3 py-2.5 text-xs font-semibold text-success">
            ...
            {rec.recommendedProject ??
              `Prioritize ${rec.category} project`}
          </div>
        </div>
      </div>
    </article>
