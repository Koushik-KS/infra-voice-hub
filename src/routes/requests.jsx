  ...
  const filtered = useMemo(
    () =>
      data.filter((r) => {
        ...
        if (
          fromDate &&
          new Date(r.createdAt ?? 0) < new Date(fromDate)
        ) {
          return false;
        }
            ...
            <div className="space-y-5 px-5 py-5">
              ...
              <dl className="grid grid-cols-2 gap-4 text-xs">
                ...
                <div>
                  ...
                  <dd className="mt-1 font-semibold">
                    {LANGUAGE_NAMES[selected.language] ??
                      selected.language ??
                      "—"}
                  </dd>
                </div>
              ...
              <p className="rounded-lg border border-border px-3 py-2.5 text-xs text-muted-foreground">
                ...
                {selected.location?.district ?? "selected region"}{" "}
                demand model and its{" "}
                {selected.category?.toLowerCase() ?? "development"}{" "}
                priority score.
              </p>
            </div>
          </aside>
